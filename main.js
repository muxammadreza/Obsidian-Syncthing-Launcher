...
function detectMobilePlatform() {
  var _a;
  if ((_a = window.app) == null ? void 0 : _a.isMobile) {
    return true;
  }
...
try {
  if (!detectMobilePlatform()) {
    ...
    fs = {
      ...
      accessSync: () => {
        throw new Error("File access not available on mobile");
      },
...
var SyncthingMonitor = class extends EventEmitter {
  constructor() {
    super(...arguments);
    this.token = null;
    this.timeout = 1;
...
Sync: ${this.monitor.fileCompletion.toFixed(1)}%`;
  ...
  async ensureConfigForPort(configDir, port) {
    if (typeof require !== "undefined") {
      ...
      const portFile = path2.join(configDir, ".syncthing-port");
      let storedPort = "";
      if (fs2.existsSync(portFile)) {
        try {
          storedPort = fs2.readFileSync(portFile, "utf8").trim();
        } catch (error) {
      ...
      if (storedPort && storedPort !== port) {
        ...
        try {
          ...
          for (const file of files) {
            const filePath = path2.join(configDir, file);
      ...
      try {
        fs2.writeFileSync(portFile, port, "utf8");
    ...
    try {
      ...
      if (typeof require !== "undefined") {
        try {
          ...
          try {
            fs2.accessSync(executablePath, fs2.constants.F_OK | fs2.constants.X_OK);
            ...
            if (platformInfo.platform !== "win32") {
              try {
                fs2.chmodSync(executablePath, "755");
      ...
      if (!asset) {
        if (platformInfo.platform === "darwin") {
          ...
          if (universalAsset) {
            ...
            new import_obsidian.Notice(`Using universal macOS build for ${platformInfo.arch} architecture`, 5e3);
        ...
        new import_obsidian.Notice(`No Syncthing release found for ${platformInfo.platform} ${platformInfo.arch}. Available assets: ${releaseInfo.assets.map((a) => a.name).join(", ")}`, 1e4);
        return false;
      }
  ...
  async downloadAndInstallAsset(asset, expectedExecutableName) {
    var _a, _b;
    try {
      new import_obsidian.Notice(`Downloading Syncthing ${((_a = asset.name.match(/v(\d+\.\d+\.\d+)/)) == null ? void 0 : _a[1]) || "latest"} for ${platformInfo.platform} ${platformInfo.arch}... Please wait.`, 8e3);
  ...
  async downloadFile(url) {
    return new Promise((resolve) => {
      const https2 = require("https");
      const urlModule2 = require("url");
      const parsedUrl = urlModule2.parse(url);
      const options = {
  ...
  async extractAndInstallSyncthing(archiveData, archiveName, executableName) {
    ...
    try {
      const fs2 = require("fs");
      const path2 = require("path");
      const syncthingDir = path2.join(this.getPluginAbsolutePath(), "syncthing");
      if (!fs2.existsSync(syncthingDir)) {
        fs2.mkdirSync(syncthingDir, { recursive: true });
      }
  ...
  async extractZip(zipData, targetDir, executableName) {
    try {
      ...
      const tempZipPath = path2.join(targetDir, "temp-syncthing.zip");
      fs2.writeFileSync(tempZipPath, zipData);
  ...
  async extractTarGz(tarData, targetDir, executableName) {
    try {
      ...
      const path2 = require("path");
      const { spawn: spawn2 } = require("child_process");
      const tempTarPath = path2.join(targetDir, "temp-syncthing.tar.gz");
      fs2.writeFileSync(tempTarPath, tarData);
      return new Promise((resolve) => {
        const extractProcess = spawn2("tar", ["-xzf", tempTarPath, "-C", targetDir]);
        extractProcess.on("close", (code) => {
          try {
            if (fs2.existsSync(tempTarPath)) {
              fs2.unlinkSync(tempTarPath);
            }
            if (code === 0) {
              this.findAndCopyExecutable(targetDir, executableName).then(resolve);
            } else {
  ...
  async findAndCopyExecutable(extractDir, executableName) {
    try {
      ...
      const execStats = fs2.statSync(executablePath);
      ...
      let finalPath;
      if (platformInfo.platform === "win32") {
        finalPath = path2.join(extractDir, "syncthing.exe");
      } else if (platformInfo.platform === "darwin") {
        finalPath = path2.join(extractDir, "syncthing-macos");
      } else {
        finalPath = path2.join(extractDir, "syncthing-linux");
      }
      console.log(`Copying executable from ${executablePath} to ${finalPath}`);
      fs2.copyFileSync(executablePath, finalPath);
      console.log(`\u2705 Executable copied successfully`);
      if (platformInfo.platform !== "win32") {
        fs2.chmodSync(finalPath, "755");
      ...
      this.cleanupExtractedFiles(extractDir, path2.basename(finalPath));
      console.log(`Syncthing executable installed to: ${finalPath}`);
      try {
        const stats = fs2.statSync(finalPath);
        const mode = stats.mode;
        console.log(`Executable permissions: ${(mode & parseInt("777", 8)).toString(8)}`);
        console.log(`File size: ${stats.size} bytes`);
        fs2.accessSync(finalPath, fs2.constants.F_OK | fs2.constants.X_OK);
  ...
  cleanupExtractedFiles(dir, keepFile) {
    try {
      ...
      for (const item of items) {
        ...
        const stat = fs2.statSync(itemPath);
        if (stat.isDirectory()) {
          fs2.rmSync(itemPath, { recursive: true, force: true });
        } else {
