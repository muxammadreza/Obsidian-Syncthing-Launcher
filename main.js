...
try {
  if (!detectMobilePlatform()) {
    ...
    http = {
      request: () => {
        throw new Error("HTTP requests not available on mobile platform");
      }
    };
    fs = {
      existsSync: () => false,
    ...
    childProcess = {
      spawn: () => {
        throw new Error("Process spawning not available on mobile");
      },
      exec: () => {
        throw new Error("Process execution not available on mobile");
      }
    };
  }
} catch (error) {
